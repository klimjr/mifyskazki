...
foreach ($arResult['ITEMS'] AS $key => $arItem) {
    ...
    // Получение нвзвания категории для элемента
    if ((int)$arItem["IBLOCK_SECTION_ID"] > 0) {
        $section = CIBlockSection::GetList(array("SORT"=>"ASC"), array("IBLOCK_ID" => $arItem["IBLOCK_ID"], "ID" => $arItem["IBLOCK_SECTION_ID"]), false, array("NAME", "SECTION_PAGE_URL"), false)->GetNext();
        $arResult['ITEMS'][$key]["SECTION_NAME"] = $section["NAME"];
        $arResult['ITEMS'][$key]["SECTION_LINK"] = $section["SECTION_PAGE_URL"];
    }

    // Ресайз изображений
    if (isset($arItem['PREVIEW_PICTURE']["ID"])) {
        if ($i == 0) {
            $arResult['ITEMS'][$key]['PREVIEW_PICTURE'] = SelfFunctions::getResizeImage($arItem['PREVIEW_PICTURE']["ID"], 263, 175);
        }
        else {
            $arResult['ITEMS'][$key]['PREVIEW_PICTURE'] = SelfFunctions::getResizeImage($arItem['PREVIEW_PICTURE']["ID"], 263, 175);
        }

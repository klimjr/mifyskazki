...
foreach ($arResult['ITEMS'] AS $key => $arItem) {
    ...
    if (isset($arItem["DISPLAY_PROPERTIES"]["LINK"]["VALUE"]) && !empty($arItem["DISPLAY_PROPERTIES"]["LINK"]["VALUE"])) {
        $arResult['ITEMS'][$key]['DETAIL_PAGE_URL'] = $arItem["DISPLAY_PROPERTIES"]["LINK"]["VALUE"];
    } elseif (isset($arItem["DISPLAY_PROPERTIES"]["ELEMENT_LINK"]["VALUE"]) && !empty($arItem["DISPLAY_PROPERTIES"]["ELEMENT_LINK"]["VALUE"])) {
        $tmp = array_shift($arItem["DISPLAY_PROPERTIES"]["ELEMENT_LINK"]["LINK_ELEMENT_VALUE"]);
        $arResult['ITEMS'][$key]['DETAIL_PAGE_URL'] = $tmp["DETAIL_PAGE_URL"];
        unset($tmp);
    } elseif (isset($arItem["DISPLAY_PROPERTIES"]["SECTION_LINK"]["VALUE"]) && !empty($arItem["DISPLAY_PROPERTIES"]["SECTION_LINK"]["VALUE"])) {
        $tmp = array_shift($arItem["DISPLAY_PROPERTIES"]["ELEMENT_LINK"]["LINK_ELEMENT_VALUE"]);
        $arResult['ITEMS'][$key]['DETAIL_PAGE_URL'] = $tmp["SECTION_PAGE_URL"];
        unset($tmp);
    }
    ...
    if (isset($arItem['PREVIEW_PICTURE']["ID"])) {
        if ($i == 0) {
            $arResult['ITEMS'][$key]['PREVIEW_PICTURE'] = SelfFunctions::getResizeImage($arItem['PREVIEW_PICTURE']["ID"], 555, 437);
        }
        else {
            $arResult['ITEMS'][$key]['PREVIEW_PICTURE'] = SelfFunctions::getResizeImage($arItem['PREVIEW_PICTURE']["ID"], 555, 437);
        }

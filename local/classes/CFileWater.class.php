...
class CFileWater extends CAllFile
{
    public static $watermark = array(
        ...
        'min_size_picture' => 200,
        'file' => '/upload/watermark_1500.png',
    );


    public static function ResizeImageGet($file, $arSize, $resizeType = BX_RESIZE_IMAGE_PROPORTIONAL, $bInitSizes = false, $arFilters = false, $bImmediate = false, $jpgQuality = false, $watermark = false)
    {
        ...
        elseif (!file_exists($io->GetPhysicalName($_SERVER["DOCUMENT_ROOT"].$cacheImageFileCheck)))
        {
            ...
            if ($bDiskQuota)
            {
                ...
                if ($watermark) {
                    if ($arSize["width"] >= 1400) {
                        self::$watermark['file'] = '/upload/watermark_1500.png';
                    } elseif ($arSize["width"] < 1400 && $arSize["width"] >= 600) {
                        self::$watermark['file'] = '/upload/watermark_600.png';
                    } else {
                        self::$watermark['file'] = '/upload/watermark_400.png';
                    }
                ...
                if ($bNeedResize && CFile::ResizeImageFile($sourceImageFile, $cacheImageFileTmp, $arSize, $resizeType, $arWatermark, $jpgQuality, $arFilters))
                {

...
<?if($arShowTabs['sections']):
    ...
    <? $res = CIBlockElement::GetElementGroups($arElement["ID"], false, array("ID", "NAME", "SECTION_PAGE_URL"));
    $elementSections = array();
    while($section = $res->GetNext()) {

        $navRes = CIBlockSection::GetNavChain($IBLOCK_ID, $section["ID"], array("ID", "NAME", "SECTION_PAGE_URL"));
        $sectionNav = array();
        while($navItem = $navRes->GetNext()) {
            $sectionNav[] = $navItem;
        }

        $sectionVendor = CIBlockSection::GetList(array("SORT"=>"ASC"), array("IBLOCK_ID" => $IBLOCK_ID, "ID" => $section["ID"]), false, array("ID", "NAME", "UF_VENDOR_NAME"), false)->Fetch();
        $section["VENDOR_NAME"] = $sectionVendor["UF_VENDOR_NAME"];
        $section["NAV"] = $sectionNav;

        $section["LINK"] = "/bitrix/admin/iblock_list_admin.php?PAGEN_1=1&SIZEN_1=20&IBLOCK_ID=".$IBLOCK_ID."&type=catalog&lang=ru&find_section_section=".$section["ID"]."&set_filter=Y&adm_filter_applied=0";

        $elementSections[] = $section;
    }
    ?>

    <? if (isset($elementSections) && !empty($elementSections)): ?>
    <tr>
        <td width="40%" class="adm-detail-valign-top"> <? echo $arElement["NAME"].' Принадлежит разделам'; ?></td>
        <td width="60%">

            <ul>
                <? foreach ($elementSections AS $elementSection): ?>
                <li>
                    <a href="<?=$elementSection["LINK"]?>"><?=$elementSection["NAME"];?></a>
                    [<?=$elementSection["ID"];?>]
                    <? if (isset($elementSection["VENDOR_NAME"]) && !empty($elementSection["VENDOR_NAME"])) echo ' ('.$elementSection["VENDOR_NAME"].')'; ?>
                </li>
